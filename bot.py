...
# [لعبة مضاربة] END
    ...
    if cmd in ("حظر", "ban"):
        ...
        return message("moderation.already_banned", "⚠️ محظور بالفعل.")
        ...
        try:
            ...
            for row in rows or []:
                ...
                if sender and sender != BOT_ID and text:
                    ...
                    cmd, arg = parts[0].lower(), (parts[1].strip() if len(parts) > 1 else "")
                    low = normalize_text(text)
                    sender_username = await username_of(sender)
                    is_master_user = await is_master(sender, sender_username)
                    is_owner = sender_username.lower() == OWNER
                    # الرسائل الخاصة والردود الخاصة مخصصة للماستر فقط.
                    # لا نرسل أي رد للمستخدم العادي.
            ...
            for rid in list(rooms):
                ...
                for m in rows or []:
                    ...
                    if text or ((rid, m.get("user_id")) in publish_pending and media_url):
                        ...
                        reply = await handle_room(rid, text, sender_id, media_url, message_type)
                        # الردود النصية للأوامر تكون للماستر فقط.
                        # أوامر/ألعاب تنفذ إرسالها بنفسها (مثل النشر والنتائج الجماعية)
                        # ولا تتأثر بهذا الشرط.
                        if reply and await is_master(sender_id, await username_of(sender_id)):
                            await room_send(rid, reply)
        except Exception:
