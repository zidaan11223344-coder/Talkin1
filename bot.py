...
def _pango_name_image(raw_text, size, fill=(255,238,199,255)):
    """Render a Talkin username with real Unicode font fallback + bidi.

    Pango (via ImageMagick) performs Arabic shaping and the Unicode bidi
    algorithm on the COMPLETE original username.  This is important for
    decorated RTL names: splitting the name into Arabic/symbol runs reverses
    the visual order and missing-glyph tofu boxes appear when one font is used
    for every character.
    """
    ...
    # Pango/fontconfig automatically falls back between these families for
    # Egyptian hieroglyphs, chess symbols, Byzantine/music symbols, Arabic,
    # Latin, etc. The original Unicode text is passed unchanged.
    families = "Noto Sans Arabic,Noto Sans Egyptian Hieroglyphs,Noto Sans Symbols2,Noto Music,DejaVu Sans,Noto Sans"
    color = "#%02x%02x%02x" % tuple(int(x) for x in fill[:3])
    markup = f'<span font_family="{families}" foreground="{color}">{text}</span>'
    out = Path("/tmp") / f"talkin_name_{uuid.uuid4().hex}.png"
    try:
        # If the project ships the small font fallback pack, expose it to
        # Pango/fontconfig as well. This makes the result independent of the
        # server's installed fonts and prevents tofu boxes for the requested
        # Egyptian/chess/music symbols.
        env = os.environ.copy()
        local_fonts = BASE_DIR / "talkin_font_fallbacks"
        if local_fonts.is_dir():
            conf = Path("/tmp") / "talkin-fonts.conf"
            conf.write_text(
                "<?xml version=\"1.0\"?>"
                "<fontconfig>"
                f"<dir>{local_fonts}</dir>"
        ...
        subprocess.run(
            [magick, "-background", "none", "-pointsize", str(int(size)),
             "-density", "144", "pango:" + markup, str(out)],
...
def _draw_name_centered(draw, center, raw_text, size, fill, max_width):
    """Draw the exact Talkin username without reversing or replacing it.

    Prefer Pango/ImageMagick because it can combine Arabic shaping with
    Unicode font fallback in one bidi-aware layout. If unavailable, use the
    older Pillow fallback, but never intentionally replace unsupported
    characters with square/tofu glyphs.
    """
    text = str(raw_text if raw_text is not None else "")
    if not text:
        return

    # First choice: one complete bidi/shaping pass over the untouched name.
    sz = int(size)
    rendered = None
    while sz >= 14:
        rendered = _pango_name_image(text, sz, fill)
        if rendered is not None:
            if rendered.width <= int(max_width):
                break
        sz -= 2
    if rendered is not None:
        # If even the smallest size is wider than the box, scale it down
        # proportionally rather than dropping or altering characters.
        if rendered.width > int(max_width):
            nh = max(1, int(rendered.height * int(max_width) / rendered.width))
            rendered = rendered.resize((int(max_width), nh), Image.LANCZOS)
        x = int(center[0] - rendered.width / 2)
        y = int(center[1] - rendered.height / 2)
        draw._image.alpha_composite(rendered, (x, y))
        return

    # Pillow fallback: keep the logical string untouched and let the actual
    # font renderer handle direction when Raqm is available.
    try:
        font = _gift_font(text, sz)
        bbox = draw.textbbox((0, 0), text, font=font, direction="rtl" if _has_arabic(text) else "ltr")
        width = bbox[2] - bbox[0]
        while width > int(max_width) and sz > 14:
            sz -= 2
            font = _gift_font(text, sz)
            bbox = draw.textbbox((0, 0), text, font=font, direction="rtl" if _has_arabic(text) else "ltr")
            width = bbox[2] - bbox[0]
        x = int(center[0] - width / 2 - bbox[0])
        y = int(center[1] - (bbox[3] - bbox[1]) / 2 - bbox[1])
